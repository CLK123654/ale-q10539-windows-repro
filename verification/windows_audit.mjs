...
const features=run(process.env.SQLITE_BIN,[':memory:',`select json_valid('{}'),row_number() over(),length(readfile('${path.join(task,'README.md').replaceAll("'","''")}'));`]);
assert(features.status===0&&/^1\|1\|[1-9]\d*/m.test(features.stdout),'SQLite business features are unavailable');
